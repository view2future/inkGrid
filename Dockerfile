# Multi-stage Dockerfile for inkGrid - Full Stack with Frontend Display
...
# Stage 2: Final image with Nginx to serve the full application
...
RUN echo 'server {
    ...
    location = /health {
        access_log off;
        return 200 "{\"status\":\"healthy\",\"service\":\"inkGrid-frontend-display\"}\n";
...
echo "==========================================="
echo ">>> 启动后端服务 (FastAPI) on port 8001..."
echo ">>> 启动前端服务 (Nginx) on port 8000..."
echo ">>> 首页将可通过 http://localhost:8000 访问"
